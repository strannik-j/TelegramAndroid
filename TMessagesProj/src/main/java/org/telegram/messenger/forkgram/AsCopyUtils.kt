...
import org.telegram.messenger.forkgram.ForkUtils.GetTLRPCMessages
...
@JvmStatic
fun GetTLRPCMessages(
        currentAccount: Int,
        messages: ArrayList<MessageObject>,
        finish: (ArrayList<TLRPC.Message>, TL_error?) -> Unit) {
    if (messages.count() == 0) {
        return;
    }
    ...
    if (channelId != 0L) {
        ...
        api.sendRequest(req) { msgResponse: TLObject?, msgErr: TL_error? ->
            val msgRes = if (msgResponse != null)
                msgResponse as messages_Messages
                else TL_messages_messagesNotModified();
...
fun SendItemsAsAlbum(
    ...
    val sendAlbum = sendRequest@{ response: TLObject?, error: TL_error? ->
        ...
        GetTLRPCMessages(currentAccount, messages, handleMessages);
    };
